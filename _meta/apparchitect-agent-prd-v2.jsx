...
const AGENTS = [
  ...
  // ── INFRA ──────────────────────────────────────────────────────────────────
  {
    ...
    desc: "Designs CI/CD pipelines, environment configurations, deployment strategies, infrastructure-as-code, release management, observability, and cost governance for the full deployment lifecycle.",
    sections: [
      mkProjectSchema(), mkIdentity("DevOpsAgent"),
      { id: "infra_scope", heading: "02 · Infrastructure Scope", fields: [
        { key: "hosting_platform",     label: "Hosting Platform(s)",              type: "list",  ph: "e.g. Vercel (frontend)\nSupabase (DB + auth + edge functions)\nCloudflare Workers (edge compute)\nRailway / Fly.io (containers)\nAWS / GCP (enterprise)" },
        { key: "env_strategy",         label: "Environment Strategy",             type: "area",  ph: "Define each environment and its purpose:\nLocal → Developer machine, full stack via Docker or local CLI\nDevelopment → Shared branch previews, seeded test data\nStaging → Production mirror, real integrations, no real user data\nProduction → Live system, real data, change-gated deploys" },
        { key: "env_parity_rules",     label: "Environment Parity Rules",         type: "list",  ph: "e.g. Staging must mirror production config exactly\nNo environment-specific code branches\nAll environments use same Docker image / build artifact\nFeature flags control behavior differences, not code forks" },
        { key: "cicd_platform",        label: "CI/CD Platform",                   type: "text",  ph: "e.g. GitHub Actions / Vercel Deploy Hooks / CircleCI / Buildkite" },
        { key: "iac_approach",         label: "Infrastructure-as-Code Approach",  type: "area",  ph: "Tool selection and rationale:\ne.g. Terraform for cloud resource provisioning\nPulumi for type-safe infra definitions\nCloudflare Wrangler for edge config\nVercel project config via vercel.json\nSupabase migrations for DB schema" },
        { key: "compute_topology",     label: "Compute Topology",                 type: "area",  ph: "Describe the full compute graph:\nEdge functions → where and what\nServerless functions → trigger types, runtimes\nContainers → orchestration, scaling rules\nBackground workers → queue system, concurrency" },
        { key: "networking",           label: "Networking & DNS",                 type: "list",  ph: "e.g. Custom domain configuration\nCDN rules and cache invalidation strategy\nDNS provider and TTL policy\nEdge routing rules\nCORS policy definition" },
      ]},
      { id: "pipeline", heading: "03 · CI/CD Pipeline Design", fields: [
        { key: "pipeline_stages",      label: "Pipeline Stages (in order)",       type: "list",  ph: "1. Install dependencies (cached)\n2. Lint (ESLint / Biome)\n3. Type check (tsc --noEmit)\n4. Unit tests (Vitest / Jest)\n5. Build (production artifact)\n6. Integration tests (against staging DB)\n7. Deploy to staging\n8. Smoke tests (critical path assertions)\n9. Manual approval gate (if required)\n10. Deploy to production\n11. Post-deploy health checks\n12. Notify team on success/failure" },
        { key: "branch_strategy",      label: "Branch & Deploy Strategy",         type: "area",  ph: "e.g. Trunk-based development:\nmain → auto-deploys to production\nfeature/* → preview deploy per PR\nhotfix/* → fast-track to production with expedited review\nRelease tags trigger versioned artifacts" },
        { key: "deploy_strategy",      label: "Deployment Strategy",              type: "area",  ph: "e.g. Blue/Green: maintain two identical environments, switch traffic atomically\nCanary: route 5% → 25% → 100% of traffic with automated rollback triggers\nRolling: replace instances progressively\nFeature flags: ship dark, enable per cohort\nRationale for which strategy and when each applies" },
        { key: "preview_deploys",      label: "Preview / Ephemeral Environments", type: "area",  ph: "How are per-PR or per-branch preview environments created, populated with test data, and destroyed? Who has access? What integrations are stubbed vs. live?" },
        { key: "rollback_procedure",   label: "Rollback Procedure",               type: "area",  ph: "Automatic rollback triggers:\n- Health check fails within 5 min of deploy\n- Error rate exceeds X% vs. baseline\n- P95 latency exceeds Y ms\n\nManual rollback:\n- Revert commit → CI triggers redeploy\n- Direct previous artifact promotion\n\nDB migration rollback:\n- Down migrations required for every up migration\n- Migration dry-run in staging before production\n\nRTO target: e.g. < 5 minutes from detection to rollback complete" },
        { key: "release_gating",       label: "Release Gating & Approval",        type: "list",  ph: "e.g. Required: 1 peer review approval on all PRs to main\nRequired: All CI checks green\nOptional: Manual QA sign-off for major features\nOptional: Security scan pass for auth/payment changes\nHotfix path: tech lead approval only, post-deploy review required" },
      ]},
      { id: "secret_config", heading: "04 · Secrets & Configuration Management", fields: [
        { key: "secret_management",    label: "Secret Management System",         type: "area",  ph: "Where secrets live and how they're accessed:\ne.g. Vercel Environment Variables for frontend/edge secrets\nSupabase Vault for DB-level secrets\nGitHub Actions Secrets for CI secrets\nNever in .env files committed to repo\nNever hardcoded in source" },
        { key: "env_var_taxonomy",     label: "Environment Variable Taxonomy",    type: "list",  ph: "Define naming conventions and required vars per env:\nPUBLIC_ prefix → safe to expose to client\nSUPABASE_URL, SUPABASE_ANON_KEY → public\nSUPABASE_SERVICE_ROLE_KEY → server-only, never client\nSTRIPE_SECRET_KEY → server-only\nANALYTICS_WRITE_KEY → server-only" },
        { key: "secret_rotation",      label: "Secret Rotation Policy",           type: "area",  ph: "e.g. API keys rotated every 90 days\nService role keys rotated every 30 days\nRotation is zero-downtime: new key deployed before old key revoked\nRotation documented in runbook and triggered by automated reminder" },
        { key: "secret_scanning",      label: "Secret Scanning",                  type: "area",  ph: "e.g. GitHub secret scanning enabled on all repos\nPre-commit hook (gitleaks or trufflehog) blocks commits with detected secrets\nCI pipeline includes secret scan step\nAlert channel for detected leaks" },
      ]},
      { id: "observability", heading: "05 · Observability & Incident Response", fields: [
        { key: "logging_strategy",     label: "Logging Strategy",                 type: "area",  ph: "Log levels (DEBUG / INFO / WARN / ERROR)\nStructured JSON logging format\nWhat is always logged: request ID, user ID, action, duration, outcome\nWhat is never logged: passwords, tokens, PII\nLog aggregation platform: e.g. Axiom / Datadog / Cloudflare Logpush / Supabase logs" },
        { key: "metrics_monitoring",   label: "Metrics & Monitoring",             type: "list",  ph: "Infrastructure metrics:\n- Server CPU / memory / disk utilization\n- Function invocation count and duration\n- DB connection pool usage\n- CDN hit/miss ratio\n\nApplication metrics:\n- Request rate and error rate\n- P50 / P95 / P99 latency per endpoint\n- Queue depth and processing lag\n- Background job success/failure rate" },
        { key: "alerting_rules",       label: "Alerting Rules",                   type: "list",  ph: "Page immediately (PagerDuty / Slack #alerts):\n- Error rate > 5% over 5 min\n- P95 latency > 2s over 5 min\n- DB connection pool > 80% utilized\n- Any 5xx on payment endpoints\n\nNotify (Slack #ops):\n- Error rate > 1% over 15 min\n- Deploy failed\n- Cron job missed scheduled run\n- Cert expiry within 14 days" },
        { key: "tracing",              label: "Distributed Tracing",              type: "area",  ph: "e.g. OpenTelemetry instrumentation on all API routes\nTrace propagation across edge functions and background jobs\nTrace sampling rate: 10% in production, 100% in staging\nTrace storage: Honeycomb / Jaeger / Datadog APM" },
        { key: "incident_runbook",     label: "Incident Response Runbook",        type: "area",  ph: "SEV1 (total outage): page on-call immediately, war room in 5 min, status page updated, comms every 30 min\nSEV2 (partial degradation): page on-call, investigate within 30 min, status page if user-visible\nSEV3 (minor issue): Slack alert, fix within 24h, post-mortem optional\nPost-mortem required for all SEV1/SEV2: blameless, 5 whys, action items with owners" },
        { key: "status_page",          label: "Status Page",                      type: "area",  ph: "Platform (e.g. Statuspage.io / Instatus / self-hosted)\nComponents listed publicly\nUpdate cadence during incidents\nHistorical uptime display\nSubscriber notifications (email/SMS/webhook)" },
      ]},
      { id: "cost_governance", heading: "06 · Cost Governance", fields: [
        { key: "cost_targets",         label: "Monthly Infrastructure Budget",    type: "area",  ph: "Budget per environment:\nDevelopment: $X/month\nStaging: $X/month\nProduction: $X/month target, $Y/month hard ceiling\n\nBudget per service tier:\nCompute: $X\nDB: $X\nStorage: $X\nObservability: $X\nExternal APIs: $X" },
        { key: "cost_alerting",        label: "Cost Alerting Thresholds",         type: "list",  ph: "e.g. Alert at 80% of monthly budget\nAlert on any single-day spike >2x rolling average\nAlert on unexpected new resource provisioning\nWeekly cost report to tech lead" },
        { key: "cost_optimization",    label: "Cost Optimization Rules",          type: "list",  ph: "e.g. Staging environments auto-pause outside business hours\nFunction timeout limits enforced to prevent runaway billing\nDB connection pooling required (no direct connections at scale)\nImage optimization and CDN caching to reduce bandwidth\nLog retention limits (7 days dev, 30 days staging, 90 days prod)\nUnused preview environments purged after 7 days" },
      ]},
      { id: "constraints", heading: "07 · Constraints & Guard Rails", fields: [
        { key: "scope_limits",         label: "Scope Limits",                     type: "list",  ph: "What does this agent NOT handle?\ne.g. Application business logic\nDatabase schema design\nAuth system implementation\nFrontend or backend code generation" },
        { key: "anti_patterns",        label: "DevOps Anti-Patterns to Flag",     type: "list",  ph: "e.g. Secrets in .env files committed to repo\nNo staging environment (deploy straight to prod)\nManual deployment steps not in CI\nNo rollback mechanism\nAll developers have production DB write access\nNo infrastructure cost monitoring\nMonolithic deploy with no feature flags\nNo health checks after deploy\nCron jobs with no alerting on missed runs\nNo structured logging (raw console.log to prod)" },
        { key: "escalation_conditions",label: "Escalation Conditions",            type: "list",  ph: "When must this agent pause and request human architectural review?\ne.g. Multi-region deployment design\nEnterprise VPC / private networking requirements\nDisaster recovery planning beyond standard rollback\nCompliance-driven infrastructure changes (SOC 2, HIPAA)" },
      ]},
      mkHandoffs(), mkEval(),
    ]
  },
